...
const BathCard = ({bathdetail}) => {
  return (
    <div className="flex justify-center items-center mt-10">
      <div className="w-[90%] h-[full] border border-gray-200 rounded shadow-md sm:ml-4">
        
        <div>
          <h1 className="text-2xl font-bold mt-10  mx-10">{bathdetail.heading}</h1>
        </div>
        <div className="flex justify-between mt-4">
          <div className="mx-10">
          <div className="flex gap-2">
          <FontAwesomeIcon icon={faStar} />
            <h1 className="mb-1">{bathdetail.ratings}</h1>
            </div>
            <div className="flex gap-3">
              <h1 className="font-bold">{bathdetail.price}</h1>
              <h1>{bathdetail.time}</h1>
            </div>
            <hr className="mt-4"/>
                <ul>
